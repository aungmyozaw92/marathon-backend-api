...
class VoucherRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        ...
        if ($pickup->sender_type == 'Merchant') {
            ...
            $sender_zone_id = isset($data['sender_zone_id']) ? $data['sender_zone_id'] : $sender_zone_id;
            $location['sender_city_id'] = $sender_city_id;
            $location['receiver_city_id'] = $data['receiver_city_id'];
            $location['receiver_zone_id'] = $data['receiver_zone_id'];
            $merchant_discount  = MerchantDiscount::customerDiscount($location)->first();
        ...
        $voucher->receiver_address = isset($data['address']) ? $data['address'] : ($customer ? $customer->address : null);
        ...
        if($pickup->pickuped_by_type === 'Agent'){
            $voucher->from_agent_id = $pickup->pickuped_by_id;
        }
        $voucher->save();
            ...
            foreach ($data['parcels'] as $key => $par) {
                ...
                if ($bus_station) {
                    ...
                    if ($pickup->sender_type == 'Merchant') {
                        if ($sender_city_id != $data['receiver_city_id'] && $sender_city->branch && $receiver_city->branch && $merchant->static_price_branch > 0) {
                        ...
                        } elseif ($sender_city_id != $data['receiver_city_id'] && (!$sender_city->branch || !$receiver_city->branch) && $merchant->static_price_diff_city > 0) {
                            $delivery_amount = $merchant->static_price_diff_city;
                        } elseif ($sender_city_id == $data['receiver_city_id'] && $merchant->static_price_same_city > 0) {
                            ...
                            if ($merchant_rate_card && !$merchant_rate_card['min_threshold'] && !$merchant_rate_card['qty_status']) {
                                ...
                                if ($merchant_rate_card['incremental_weight'] > 0) {
                                    // $round_weight = (int) (1 * ceil($data['weight'] / 1));
                                    // $incremental_amount = ($round_weight - (int)$merchant_rate_card['from_weight']) * $merchant_rate_card['incremental_weight'];
                    ...
                    if ($sender_city_id != $receiver_city_id) {
                        ...
                        if ($agent) {
                            ...
                            $agent = ($pickup->sender_type == 'Merchant') ? $sender_city->agent : City::find($sender_city_id)->agent;
                            if ($agent) {
                                $weight = ($par['weight'] > $cbm->support_weight) ? ($par['weight'] - $cbm->support_weight) * $base_data->agent_salt : 0;
                                $agent_fee = $agent->delivery_commission + $weight + $zone_agent_rate;
                            }
        ...
        //$voucher->receiver_amount_to_collect = computeRoundingAmount($data['receiver_amount_to_collect']);
    ...
    public function update(Voucher $voucher, array $data)
    {
        ...
        if ($pickup->sender_type == 'Merchant') {
            ...
            if ($merchant->is_discount) {
                if (($sender_city_id != $data['receiver_city_id'] && $sender_city->branch && $receiver_city->branch && $merchant->static_price_branch > 0)
                    ...
                    $merchant_discount    = $this->get_merchant_discount($merchant, $data, $voucher->created_by_type);
                }
            }
            ...
            $location['sender_city_id'] = $sender_city_id;
            $location['receiver_city_id'] = $receiver_city_id;
            $location['receiver_zone_id'] = $receiver_zone_id;
            $merchant_discount  = MerchantDiscount::customerDiscount($location)->first();
        ...
        if (isset($data['parcels'])) {
            foreach ($data['parcels'] as $key => $par) {
                ...
                $global_scale_id = isset($par['global_scale_id']) ? $par['global_scale_id'] : $par['global_scale']['id'];
                $cbm = GlobalScale::findOrFail($global_scale_id);
                $data_weight = isset($par['weight']) ? $par['weight'] : 1;
                if ($data_weight > $cbm->max_weight) {
                    $responses = [
        ...
        $voucher->remark = isset($data['remark']) ? $remark : $voucher->remark;
        ...
        $voucher->thirdparty_invoice = isset($data['thirdparty_invoice']) ? $data['thirdparty_invoice'] : $voucher->thirdparty_invoice;
        if (isset($data['postpone_date'])) {
            ...
            $voucher->postpone_actor_type = 'Staff';
        }
        ...
        if (isset($data['parcels'])) {
            ...
            foreach ($data['parcels'] as $key => $par) {
                //updating parcel item price
                if (count($data['parcels']) == 1 && isset($data['total_item_price']) && $data['total_item_price']) {
                    $par["parcel_items"][0]['item_price'] = $data['total_item_price'];
                }

                $global_scale_id = isset($par['global_scale_id']) ? $par['global_scale_id'] : $par['global_scale']['id'];
                $cbm = GlobalScale::findOrFail($global_scale_id);
                // $cbm = GlobalScale::findOrFail(isset($par['global_scale_id']) ? $par['global_scale_id'] : 1 );
                if ($bus_station) {
                    $base_data = BusDropOff::where('gate_id', $sender_gate_id)->where('global_scale_id', $cbm->id)->firstOrFail();
                    $weight = ($par['weight'] > $cbm->support_weight) ? ($par['weight'] - $cbm->support_weight) * $base_data->salt : 0;
                    if ($pickup->sender_type == 'Merchant') {
                        if ($sender_city_id != $receiver_city_id && $sender_city->branch && $receiver_city->branch && $merchant->static_price_branch > 0) {
                            ...
                            $data['weight'] = $par['weight'];
                            $merchant_rate_card    = $this->get_merchant_rate_card($merchant_associate, $data, $voucher->created_by_type);
    ...
    {
        // Log::info("Agent Waybill Confirm Voucher Close");
        if (!$voucher->is_closed) {
            ...
            $journalRepository->create($voucher);
    ...
    public function return(Voucher $voucher): Voucher
    {
        ...
        $voucher->pending_returning_actor_type = 'Staff';
    ...
    public function return_backup(Voucher $voucher): Voucher
    {
        $journalRepository = new JournalRepository();
    ...
    public function update_status(Voucher $voucher, array $data): Voucher
    {
        ...
        if (isset($data['delivery_status_id']) && $data['delivery_status_id'] == 10) {
            $waybill_voucher_note = null;
            if (isset($data['waybill_voucher_note'])) {
                $note_string = implode(',', $data['waybill_voucher_note']);
                $separated_note = str_replace(',', '|', $note_string);
                $waybill_voucher_note = getConvertedString($separated_note);
            }
            WaybillVoucher::where('waybill_id', $data['waybill_id'])
            ...
            auth()->user()->messages()->save($messages);
        }
        // swip cities if agent return voucher
        // if($data['delivery_status_id'] ==9) {
        //     $voucher->origin_city_id = $voucher->receiver_city_id;
        //     $voucher->receiver_city_id = $voucher->sender_city_id;
        //     $voucher->sender_city_id = $voucher->origin_city_id;
        // }
        // counter record for can't deliver
        if (isset($data['delivery_status_id']) && $data['delivery_status_id'] == 10) {
            $voucher->delivery_counter += 1;
        }
        if (isset($data['delivery_status_id']) && $data['delivery_status_id'] == 2) {
    ...
    public function get_merchant_rate_card($merchant_associate, array $data, $type)
    {
        $bus_station = isset($data['bus_station']) ? $data['bus_station'] : null;
        // calculate normal delivery amount and merchant discount for bus dropoff and normal voucher
        if ($bus_station) {
            ...
            $locations = [
                'qty_rate_card' => isset($data['qty_rate_card']) ? $data['qty_rate_card'] : null,
                'receiver_zone_id' => isset($data['receiver_zone_id']) ? $data['receiver_zone_id'] : null,
                'sender_zone_id'   => isset($data['sender_zone']) ? $data['sender_zone'] : null,
                'receiver_city_id'     => $data['receiver_city_id'],
                'sender_city_id'       => $data['sender_city'],
            ...
            if (!$merchant_rate_cards) {
                $merchant_rate_cards = $merchant_associate->merchant_rate_cards()->normalVoucher()
                    ->allCityRateCard($locations)
                    ->allZoneRateCard($locations)
                    ->weightRateCard($locations)
                    ->first();
            }
            // dd($merchant_rate_cards);
        }

        if ($merchant_rate_cards['platform'] == 'All' || $merchant_rate_cards['platform'] == 'Web'  || $type == 'Merchant') {
     ...
     * Upload Attachment
     */
    public function upload($voucher, $file): Voucher
    {
        /**
