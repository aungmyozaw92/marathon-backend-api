...
class IncompleteVoucherRepository extends BaseRepository
{
    /**
     * @return string
     */
    ...
    public function create(array $data)
    {
        ...
        if (isset($data['parcels'])) {
            foreach ($data['parcels'] as $key => $par) {
                $par['global_scale_id'] = isset($par['global_scale_id']) ? $par['global_scale_id'] : 1;
                $par['weight'] = isset($par['weight']) ? $par['weight'] : 1;
            ...
            $parcels = [
                'global_scale_id' => $global_scale_id,
        ...
        $voucher->total_item_price = $parcel_total_price;
     ...
     * @return mixed
     */
    public function update_receiver(Voucher $voucher, array $data)
    {
        $extra_or_discount = 0;
        $bus_fee = 0;
        $sender_id       = isset($data['sender_id']) ? $data['sender_id'] : auth()->user()->city_id;
        ...
        if ($merchant->is_discount) {
            if (($sender_city_id != $receiver_city_id && $sender_city->branch && $receiver_city->branch && $merchant->static_price_branch > 0)
                || ($sender_city_id != $receiver_city_id && (!$sender_city->branch || !$receiver_city->branch) && $merchant->static_price_diff_city > 0)
                || ($sender_city_id == $receiver_city_id && $merchant->static_price_same_city > 0)
            ) {
                $merchant_discount = null;
            } else {
        ...
        $receiver = [
            'name'    => isset($data['receiver_name']) ? $data['receiver_name'] : null,
            'phone'   => isset($data['receiver_phone']) ? $data['receiver_phone'] : null,
            'other_phone' => isset($data['other_phone']) ? $data['other_phone'] : null,
            'address' => isset($data['receiver_address']) ? $data['receiver_address'] : null,
            'city_id' => isset($data['receiver_city_id']) ? $data['receiver_city_id'] : null,
            'zone_id' => isset($data['receiver_zone_id']) ? $data['receiver_zone_id'] : null,
        ];
        ...
        $voucher->receiver_city_id = $data['receiver_city_id'];
        $voucher->receiver_zone_id = $data['receiver_zone_id'];
        ...
        if ($voucher->isDirty()) {
            ...
            $voucher->updated_by = auth()->user() ? auth()->user()->id : $voucher->created_by_merchant->id;
        ...
        if ($voucher->parcels()->count() > 0) {
            ...
            foreach ($voucher->parcels as $key => $par) {
                $cbm = GlobalScale::findOrFail($par->global_scale_id);
                if ($bus_station) {
                    $base_data = BusDropOff::where('gate_id', $sender_gate_id)->where('global_scale_id', $cbm->id)->firstOrFail();
                ...
                $parcel_data['coupon_price']   = $coupon_amount;
                $parcel_data['discount_type_id'] = $discount_type_id;
                $parcel_data['coupon_associate_id'] = $coupon_associate_id;
                $parcel_data['agent_fee'] = $agent_fee;
                $parcel_data['discount_price'] = $cal_label_price['discount_price'];
                //$parcel_data['cal_parcel_price'] = $cal_label_price['cal_parcel_price'];
                $parcel_data['cal_delivery_price'] = $cal_label_price['cal_delivery_price'];
                $parcel_data['cal_gate_price'] = $cal_label_price['cal_gate_price'];
                // $parcel_data['label_parcel_price'] = $cal_label_price['label_parcel_price'];
                $parcel_data['label_delivery_price'] = $cal_label_price['label_delivery_price'];
                $parcel_data['label_gate_price'] = $cal_label_price['label_gate_price'];
                // $par['origin_lwh'] = isset($par['origin_lwh']) ? $par['origin_lwh'] : null;
                // $par['origin_weight'] = isset($par['origin_weight']) ? $par['origin_weight'] : null;

                // $parcel_data = Parcel::findOrFail($par->id);
                $parcel = $parcelRepository->update($par, $parcel_data);
    ...
    public function update(Voucher $voucher, array $data)
    {
		...
		// remove firestore
        // $customer ?  $customer->voucherFireStore($voucher) : null;
        if ($receiver_city_id) {
            $this->receiver_id = $customer->id;
            $voucher->receiver_id = $this->receiver_id;

            $voucher->receiver_name = isset($data['receiver_name']) ? $data['receiver_name'] : null;
            $voucher->receiver_phone = isset($data['receiver_phone']) ? $data['receiver_phone'] : null;
            $voucher->receiver_other_phone = isset($data['other_phone']) ? $data['other_phone'] : null;
            $voucher->receiver_address = isset($data['receiver_address']) ? $data['receiver_address'] : null;
        ...
        if ($voucher->isDirty()) {
            ...
            $voucher->is_complete = $voucher->receiver_id ? 1 : 0;
            $voucher->save();
            // if ($voucher->pickup_id !== null) {
            //     $voucher->voucherPickupFire('remove_pickup_voucher');
            // }
    ...
    public function get_merchant_discount($merchant, array $data)
    {
        ...
        if ($bus_station) {
            ...
            $locations = [
                ...
                'receiver_city_id'     => isset($data['receiver_city_id']) ? $data['receiver_city_id'] : null,
                'sender_city_id'       => isset($data['sender_city']) ? $data['sender_city'] : null
            ];
    ...
    public function get_merchant_rate_card($merchant_associate, array $data)
    {
        ...
        if ($bus_station) {
            ...
            $locations = [
                ...
                'sender_zone_id'   => isset($data['sender_zone']) ? $data['sender_zone'] : null,
                'receiver_city_id'     => isset($data['receiver_city_id']) ? $data['receiver_city_id'] : null,
                'sender_city_id'       => isset($data['sender_city_id']) ? $data['sender_city_id'] : null,
                'weight'       => $data['weight'],
            ];
        ...
        if ($merchant_rate_cards['platform'] == 'All' || $merchant_rate_cards['platform'] == 'MerchantDashboard') {
