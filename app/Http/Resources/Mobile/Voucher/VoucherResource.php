...
namespace App\Http\Resources\Mobile\Voucher;
...
class VoucherResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $credit_amount = $this->journals->where('status', 1)
            ->where('credit_account_id', auth()->user()->account->id)
            ->where('debit_account_id', 1)->first();
        $debit_amount = $this->journals->where('status', 1)
            ->where('debit_account_id', auth()->user()->account->id)
            ->where('credit_account_id', 1)->first();
        if ($debit_amount) {
        ...
        if($this->delivered_date){
            $startDay = \Carbon\Carbon::parse($this->pickup->pickup_date);
            $endDay = \Carbon\Carbon::parse($this->delivered_date);
            // $startTime = \Carbon\Carbon::parse($this->pickup->pickup_date->format('H:i:s'));
            // $endTime = \Carbon\Carbon::parse($this->delivered_date->format('H:i:s'));
        ...
        return [
            ...
            'lead_time' => $lead_time,
            'qr_code' => $qr_code,
            'finance_amount' => $credit_amount - $debit_amount,
